// Shield - CÃ³digo injetado diretamente na pÃ¡gina via chrome.scripting.executeScript
// Este arquivo NÃƒO deve ser ofuscado para funcionar corretamente
(function() {
  ...
  // Handler global nomeado (persiste entre execuÃ§Ãµes)
  ...
  if (enable) {
    ...
    const oldOverlay = document.getElementById('lovablex-shield-input');
    if (oldOverlay) oldOverlay.remove();
    const existingOverlay = document.getElementById('lovablex-shield-overlay');
    if (existingOverlay) existingOverlay.remove();

    // Injetar CSS de animaÃ§Ã£o
    if (!document.getElementById('lovablex-shield-css')) {
      const style = document.createElement('style');
      style.id = 'lovablex-shield-css';
      style.textContent = '\
    ...
    if (found > 0) {
      ...
      if (firstBlocked) {
        ...
        shield.id = 'lovablex-shield-overlay';
        ...
        shield.innerHTML = '<span style="background:rgba(220,38,38,0.9);color:white;padding:4px 14px;border-radius:8px;font-size:12px;font-weight:700;font-family:system-ui,sans-serif;pointer-events:none;white-space:nowrap;">\u{1F512} Use a extensÃ£o Lovablex</span>';
        document.body.appendChild(shield);

        // FunÃ§Ã£o de posicionamento responsivo
        ...
        // ResizeObserver para mudanÃ§as de layout
        try {
        ...
        // Interval de seguranÃ§a para recalcular posiÃ§Ã£o
    ...
    // Remover overlay e listeners de posiÃ§Ã£o
    var overlay = document.getElementById('lovablex-shield-overlay');
    if (overlay) overlay.remove();
    var oldOverlay2 = document.getElementById('lovablex-shield-input');
    if (oldOverlay2) oldOverlay2.remove();
    var css = document.getElementById('lovablex-shield-css');
    if (css) css.remove();
